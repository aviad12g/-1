#!/usr/bin/env python3
import argparse,csv,json,math,os,random,subprocess,time
...
def loss_grad(y,vv,ss,occ,lam=0.0):
    q=(1-ss*y[vv])*.5
    gl=np.empty_like(q); gl[:,0]=(-.5*ss[:,0])*q[:,1]*q[:,2]; gl[:,1]=(-.5*ss[:,1])*q[:,0]*q[:,2]; gl[:,2]=(-.5*ss[:,2])*q[:,0]*q[:,1]
...
def vector_prior(cs,n,seed,restarts=2,steps=80):
    ...
    for r in range(restarts):
        ...
        for t in range(steps):
            g=loss_grad(y,vv,ss,occ,.0004*(t/max(1,steps-1))**3); eta=1/(1+.012*t)
    ...
    w=np.array([math.exp(-1.15*(u-best)) for u,_ in sel]); Y=np.stack([y for _,y in sel]); c=(w[:,None]*Y).sum(0)/w.sum()
...
def verify(cs,mod): return mod is not None and all(any((l>0 and mod[abs(l)]>0) or (l<0 and mod[abs(l)]<0) for l in c) for c in cs)

def run(binpath,cnf,timeout,hints=None,threshold=.9):
    env=os.environ.copy()
    if hints: env['KISSAT_V11_HINTS']=str(hints); env['KISSAT_V11_THRESHOLD']=str(threshold)
    else: env.pop('KISSAT_V11_HINTS',None); env.pop('KISSAT_V11_THRESHOLD',None)
    t=time.perf_counter()
    try:
        r=subprocess.run([binpath,f'--time={timeout}',str(cnf)],capture_output=True,text=True,env=env,timeout=timeout+5)
        ...
        hint_loaded=('V11b loaded' in text)
        if status=='UNKNOWN' and wall<0.05: print('FAST_UNKNOWN',r.returncode,text[-1000:],flush=True)
        return dict(status=status,wall_s=wall,hint_loaded=hint_loaded,model=model(text,n))
    except subprocess.TimeoutExpired:
        return dict(status='TIMEOUT',wall_s=time.perf_counter()-t,hint_loaded=bool(hints),model=None)
...
def main():
    ...
    for n in sizes:
      for j in range(a.seeds):
        ...
        hp=root/f'n{n}_{seed}_vector.hints'; rp=root/f'n{n}_{seed}_random.hints'; write_hints(hp,phase,conf); write_hints(rp,rphase,conf)
        ...
        for name,hints,th,pre in variants:
            ...
            if hints and res['status'] not in ('TIMEOUT','UNKNOWN') and not res['hint_loaded']: errors.append((n,seed,name,'hint-not-loaded'))
            if res['status']=='SAT': valid=verify(cs,mod); errors += [] if valid else [(n,seed,name,'invalid-model')]
            row={'n':n,'m':len(cs),'seed':seed,'variant':name,'threshold':th if th is not None else '', 'hint_fraction':float((conf>=th).mean()) if th is not None else 0.0,'vector_prep_s':pre,'vector_best_unsat':best,'model_valid':valid,**res}; row['total_s']=row['wall_s']+pre
            rows.append(row); solved[name]=row['status']; print('ROW',json.dumps(row),flush=True)
