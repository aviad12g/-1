...
def run(binpath,cnf,timeout,mode,conf,cap,max_size,max_glue):
    ...
    try:
        ...
        return dict(status='TIMEOUT',wall_s=time.perf_counter()-t,model=None,decisions=None,conflicts=None,
                    v12_seen_conflicts=None,v12_stored_clauses=None,v12_retrievals=None,v12_hint_uses=None,v12_random_uses=None)
...
def main():
    ...
    ap.add_argument('--timeout',type=int,default=8); ap.add_argument('--confs',default='.65,.75'); ap.add_argument('--cap',type=int,default=256)
    ...
    for n in sizes:
      for j in range(a.seeds):
        ...
        for name,mode,c in variants:
            ...
            row={'n':n,'m':len(cs),'seed':seed,'variant':name,'mode':mode,'conf_threshold':c,'model_valid':valid,**res}
    ...
    Path('v12_summary.json').write_text(json.dumps({'rows':len(rows),'errors':errors},indent=2))
    print('ERRORS',errors)
