...
from v18_router import route, RULE_TEXT
PREFIX='SAT-RAG-V15|'; START=350; STOP=600; MAX_DOWNLOAD=25_000_000; MAX_RAW=120_000_000
...
def download(h,p):
    ...
    raw=normalize(data);p.write_bytes(raw);return {'download':'ok','download_bytes':len(data),'cnf_bytes':len(raw),'sha256_cnf':hashlib.sha256(raw).hexdigest()}

def main():
    shard=int(os.environ.get('SHARD','0')); nshards=int(os.environ.get('NSHARDS','10')); root=Path('v18_blind');root.mkdir(exist_ok=True)
    ...
    rows=list(csv.DictReader(open(root/'selected.csv')));seen={x.strip() for x in Path('v15_seen_hashes.txt').read_text().splitlines() if x.strip()}
    cand=[r for r in rows if r['hash'] not in seen];cand.sort(key=lambda r:hashlib.sha256((PREFIX+r['hash']).encode()).hexdigest());chosen=cand[START:STOP]
    out=[]
    for rank,r in enumerate(chosen,start=START):
        if rank%nshards!=shard:continue
        p=root/(r['hash']+'.cnf');rec={'global_rank':rank,'hash':r['hash'],'author':r['author'],'family':r['family'],'known':r['result'],'selection_digest':hashlib.sha256((PREFIX+r['hash']).encode()).hexdigest(),'v18_rule':RULE_TEXT}
        try:
